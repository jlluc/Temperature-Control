...
int DS1621_Read_Temp()
{
    ...
    char Device = 0x48;                    // Initialize the device
    char Data_Ret;                    // Data return
    char Cmd = READ_TEMP;             // Set cmd = read_time from the device DS1621
    I2C_Write((Device << 1) | 0);     // Device address
    I2C_Write(Cmd);                   // Send register address
    I2C_ReStart();                    // Restart I2C
    I2C_Write((Device << 1) | 1);     // Initialize data read
    Data_Ret = I2C_Read(NAK);         // Read the Data_Return
...
void DS3231_Read_Time()
{
    ...
    I2C_ReStart();                      // Restart I2C
    I2C_Write((Device << 1) | 1);       // Initialize data read
    second = I2C_Read(ACK);             // Read and store seconds
    minute = I2C_Read(ACK);             // Read and store minutes
    hour = I2C_Read(ACK);               // Read and store hour
    dow = I2C_Read(ACK);                // Read and store day of week
    day = I2C_Read(ACK);                // Read and store date
    month = I2C_Read(ACK);              // Read and store month
    year = I2C_Read(NAK);               // Read and store year
    I2C_Stop();                         // Stop sequencing
}
